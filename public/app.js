...
function setButtonLoading(provider, on) {
  const btn   = document.getElementById(`btn-${PM[provider]}`);
  const arrow = document.getElementById(`arrow-${PM[provider]}`);
  if (!btn) return;
  btn.disabled = on;
...
function sparkle(x, y) {
  for (let i = 0; i < 14; i++) {
    ...
    el.style.cssText = `left:${x}px;top:${y}px;--tx:${Math.cos(angle)*dist}px;--ty:${Math.sin(angle)*dist}px;background:hsl(${Math.random()*80+160},100%,65%)`;
...
async function fetchMe() {
  ...
  try {
    const res = await apiFetch('/auth/me', {}, 20000);
...
async function tryRefresh() {
  try {
    const res = await apiFetch('/auth/refresh', { method:'POST' }, 20000);
...
function renderSidebar() {
  ...
  panel.innerHTML = `
    ...
    <div class="sb-list">
      ${ORDER.map((p,i) => {
        const sess = s[p];
        const c    = COLORS[p];
        return `<div class="sb-item${sess?' connected':''}" data-p="${p}" title="${sess ? sess.name+' · '+timeAgo(sess.ts) : 'Войти через '+PROVIDER_LABELS[p]}">
          <div class="sb-icon" style="--c:${c}">${ICONS[p]}${sess?'<span class="sb-dot"></span>':''}</div>
          <div class="sb-info">
            <div class="sb-name">${PROVIDER_LABELS[p]}</div>
            <div class="sb-sub" style="color:${sess?'var(--ok)':'var(--muted2)'}">
              ${sess ? timeAgo(sess.ts) : `клавиша ${i+1}`}
            </div>
          </div>
          ${sess && sess.avatar
            ? `<img class="sb-av" src="${sess.avatar}" onerror="this.style.display='none'" alt="">`
            : sess
              ? `<div class="sb-av sb-av-init" style="background:${c}22;color:${c}">${(sess.name||'?')[0].toUpperCase()}</div>`
  ...
  panel.querySelectorAll('.sb-item').forEach(el => {
    el.addEventListener('click', () => {
      ...
      const btnId = { vk:'btn-vk', yandex:'btn-ya', mailru:'btn-mail' }[p];
      const btn = document.getElementById(btnId);
      if (btn && !btn.disabled) {
        btn.classList.add('highlight-pulse');
        setTimeout(() => { btn.classList.remove('highlight-pulse'); login(p); }, 150);
      }
    });
  });
}
...
function renderUpdateWidget() {
  ...
  el.innerHTML = `
    ...
    ${upd ? `<div class="upd-sep">·</div><div class="upd-info"><div class="upd-action">${upd.action}</div><div class="upd-ago">${timeAgo(upd.time)}</div></div>` : ''}`;
}
...
document.addEventListener('keydown', e => {
  ...
  if (map[e.key] && document.getElementById('login-section').style.display !== 'none') {
    login(map[e.key]);
  }
...
async function checkAuth() {
  // Показываем "пробуждение" через 4 секунды если сервер не отвечает
  const wakeTimer = setTimeout(() => setLoadingMessage('Сервер просыпается... ~30 сек'), 4000);

  try {
    if (getToken()) {
      const u = await fetchMe();
      if (u) { clearTimeout(wakeTimer); renderUser(u); return; }
      clearToken();
    }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Читаем токен/ошибку из fragment (#) ИЛИ query string (совместимость)
  ...
  if (token) {
    setToken(token);
    // Показываем "загрузка профиля" пока fetchMe отвечает
    setLoadingMessage('Загрузка профиля...');
    const wakeTimer = setTimeout(() => setLoadingMessage('Получаем данные...'), 3000);
    fetchMe().then(u => {
      clearTimeout(wakeTimer);
      if (u) { renderUser(u); }
